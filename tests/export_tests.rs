...
fn test_empty_dataframe_skip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exporter = Workbook::new()?
        .insert(empty_df, Some("空表".into()), None, None)? // 應該跳過
        .insert(normal_df, Some("正常表".into()), None, None)?; // 應該成功

    // 驗證：最終隊列中應該只有 1 張表
    assert_eq!(exporter.sheet_count(), 1);
    assert_eq!(exporter.get_sheet_name(0).unwrap(), "正常表");
    Ok(())
}
...
fn test_invalid_name_auto_fix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exporter = Workbook::new()?
        .insert(df, invalid_name, None, None)?;

    // 驗證：名稱被重置為默認格式
    assert_eq!(exporter.get_sheet_name(0).unwrap(), "Sheet 1");
    Ok(())
}
...
fn test_duplicate_name_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = Workbook::new()?
        .insert(df1, Some("重複名".into()), None, None)?
        .insert(df2, Some("重複名".into()), None, None);
...
fn test_unknown_style_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = Workbook::new()?
        .insert(df, None, Some(style_map), None);
...
fn test_style_coordinate_cleanup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exporter = Workbook::new()?
        .insert(df, Some("坐標測試".into()), Some(style_map), None)?;

    // 驗證：style_map 最終變為 None（因為唯一的坐標被 retain 剔除了）
    assert!(exporter.get_sheet_style_map_simple(0).is_none());
    Ok(())
}