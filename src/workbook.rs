...
impl Workbook {
    ...
    /// 3. 全局默认样式 → styles.get("default")
    /// 4. 系统回退样式 → Format::default()
    /// ```
    ///
    /// ## 工作表处理流程
    /// ```text
    ...
    ///
    /// ## 多层样式查找
    /// 采用四级样式查找机制确保样式应用的灵活性和一致性：
    /// 1. **单元格特定样式**：最高优先级，精确到具体单元格
    /// 2. **表头默认样式**：当无单元格样式时自动应用
    /// 3. **全局默认样式**：系统预置的基础样式
    /// 4. **系统回退样式**：最终保障，确保不会出现样式缺失
    ///
    /// # 数据类型转换策略
    ///
    /// ## 数值类型处理
    /// 将所有整数类型统一转换为 f64 格式写入 Excel：
    /// - Int8/Int16/Int32/Int64 → f64
    /// - UInt32 → f64
    /// - Float32 → f64 (精度保持)
    /// - Float64 → 直接使用
    ///
    ...
    /// ```text
    /// Polars 天数 → Excel 天数
    /// days + 25569 = excel_days
    /// ```
    ///
    /// ### Datetime 类型转换
    /// ```text
    /// Unix 秒数 → Excel 天数
    /// (seconds / 86400) + 25569 = excel_days
    /// ```
    ///
    /// ## 空值处理
    /// 使用 `write_blank` 方法保持样式一致性，避免单元格显示空白。
    ///
    /// ## 其他类型处理
    /// 通过 `format!` 宏转换为字符串后写入。
    ///
    /// # 错误处理策略
    ///
    /// ## 全面错误传播
    /// 所有可能的错误都会通过 `?` 操作符传播：
    /// * `rust_xlsxwriter::XlsxError` - Excel 操作错误
    /// * `polars::PolarsError` - 数据访问错误
    /// * `std::io::Error` - 文件系统错误
    ///
    /// ## 原子性保证
    /// 如果保存过程中任何环节失败，整个操作会回滚，
    /// 确保不会产生损坏的 Excel 文件。
    ///
    /// # 性能特征
    ///
    ...
    /// - **样式查找缓存**：预取常用样式引用
    /// - **类型转换优化**：直接映射减少中间转换
    /// - **批量写入**：利用 rust_xlsxwriter 的批量处理能力
    ///
    /// # 安全考虑
    ///
    /// ## 文件系统安全
